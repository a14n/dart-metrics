...
main() {
  ...
  group('metric registry', () {
    ...
    test('concatenates strings to form a dotted name', () {
      expect(MetricRegistry.name(['one', 'two', 'three']),
    ...
    test('removes metrics matching a filter', () {
      ...
      expect(registry.names,
          unorderedEquals(['timer-1', 'timer-2', 'histogram-1']));
...
void registeringAMetricTriggersANotification(Metric m) {
  final registry = new MetricRegistry();
...
void removingAMetricTriggersANotification(Metric m) {
  final registry = new MetricRegistry();
