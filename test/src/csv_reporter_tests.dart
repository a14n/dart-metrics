...
import '../lib/mock.dart';

import 'csv_reporter_tests.mocks.dart';

@GenerateMocks([Clock, Counter, Histogram, Snapshot, Meter, Timer, Gauge],
  customMocks: [MockSpec<MetricRegistry>(as: #MockMetricRegistry, fallbackGenerators: {#register: registryRegisterMockShim})
])
main() {
  group('csv reporter', () {
...
'''));
    ...
    test('reports timer values', () {
      ...
      final snapshot = new MockSnapshot();
      when(snapshot.max).thenReturn(const Duration(milliseconds: 100).inMicroseconds);
      when(snapshot.mean).thenReturn(const Duration(milliseconds: 200).inMicroseconds.toDouble());
      when(snapshot.min).thenReturn(const Duration(milliseconds: 300).inMicroseconds);
      when(snapshot.stdDev).thenReturn(const Duration(milliseconds: 400).inMicroseconds.toDouble());
      when(snapshot.median).thenReturn(const Duration(milliseconds: 500).inMicroseconds.toDouble());
      when(snapshot.get75thPercentile()).thenReturn(const Duration(milliseconds: 600).inMicroseconds.toDouble());
      when(snapshot.get95thPercentile()).thenReturn(const Duration(milliseconds: 700).inMicroseconds.toDouble());
      when(snapshot.get98thPercentile()).thenReturn(const Duration(milliseconds: 800).inMicroseconds.toDouble());
      when(snapshot.get99thPercentile()).thenReturn(const Duration(milliseconds: 900).inMicroseconds.toDouble());
      when(snapshot.get999thPercentile()).thenReturn(const Duration(milliseconds: 1000).inMicroseconds.toDouble());
